...
const App = () => {
  ...
  const [showNoteInput, setShowNoteInput] = useState(false); // State to toggle note input visibility
  const [showNotes, setShowNotes] = useState(false); // State to show notes after clicking "Get Started"

  // Fetch notes from Appwrite database when "Get Started" is clicked
  useEffect(() => {
    if (showNotes) {
      fetchNotes(); // Fetch notes when "Get Started" is clicked
    }
  }, [showNotes]);
  ...
  async function saveNote() {
    ...
    try {
      ...
      fetchNotes(); // Re-fetch the notes to display the new note
    } catch (error) {
  ...
  // Handle the "Get Started" button click to show notes and input area
  const handleGetStartedClick = () => {
    setShowNotes(true); // Show notes and input area after clicking the button
    setShowNoteInput(true); // Show the note input field
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100 p-4">
      ...
      {!showNotes && (
        <div className="text-center mt-80">
          <button
            ...
            className="bg-indigo-600 text-white font-semibold py-2 px-6 rounded-xl hover:bg-indigo-700 transition duration-200"
          >
            Let's Get Started
          </button>
        </div>
      ...
      {/* Note Input (Only visible after clicking "Let's Get Started") */}
